...
import appsaabaza
from appsaabaza.apps.common.http import URL
from appsaabaza.apps.common.generics import FormView, MultipleObjectFormActionView
from appsaabaza.apps.common.mixins import RedirectionMixin
from appsaabaza.apps.common.settings import (
    setting_home_view, setting_project_title, setting_project_url
)
from appsaabaza.apps.user_management.permissions import permission_user_edit
...
class AppsaabazaLoginView(StrongholdPublicMixin, LoginView):
    """
    ...
    def form_valid(self, form):
        result = super(AppsaabazaLoginView, self).form_valid(form=form)
...
class AppsaabazaLogoutView(LogoutView):
    """No current change or overrides, left here for future expansion"""


class AppsaabazaPasswordChangeDoneView(PasswordChangeDoneView):
...
class AppsaabazaPasswordChangeView(PasswordChangeView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        return super(AppsaabazaPasswordChangeView, self).dispatch(*args, **kwargs)


class AppsaabazaPasswordResetCompleteView(StrongholdPublicMixin, PasswordResetCompleteView):
    extra_context = {
        'appearance_type': 'plain'
    }
...
class AppsaabazaPasswordResetConfirmView(StrongholdPublicMixin, PasswordResetConfirmView):
    extra_context = {
        'appearance_type': 'plain'
    }
...
class AppsaabazaPasswordResetDoneView(StrongholdPublicMixin, PasswordResetDoneView):
    extra_context = {
        'appearance_type': 'plain'
    }
...
class AppsaabazaPasswordResetView(StrongholdPublicMixin, PasswordResetView):
    email_template_name = 'authentication/password_reset_email.html'
    extra_context = {
        'appearance_type': 'plain'
    }
    extra_email_context = {
        'project_copyright': appsaabaza.__copyright__,
        'project_license': appsaabaza.__license__,
    ...
    def get(self, *args, **kwargs):
        ...
        return super(AppsaabazaPasswordResetView, self).get(*args, **kwargs)

    def post(self, *args, **kwargs):
        if setting_disable_password_reset.value:
            return redirect(to=setting_home_view.value)
        return super(AppsaabazaPasswordResetView, self).post(*args, **kwargs)
