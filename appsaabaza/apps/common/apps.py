...
from .links import (
    ...
    link_object_error_list_clear, link_setup, link_store, link_support, link_tools
)
...
class AppsaabazaAppConfig(apps.AppConfig):
    app_namespace = None
    app_url = None

    def ready(self):
        logger.debug('Initializing app: %s', self.name)
        from appsaabaza.urls import urlpatterns as appsaabaza_urlpatterns
...
class CommonApp(AppsaabazaAppConfig):
    app_namespace = 'common'
    app_url = ''
    has_rest_api = True
    has_tests = True
    name = 'appsaabaza.apps.common'
    ...
    def setup_auto_logging(self):
        if setting_auto_logging.value:
            ...
            logging_configuration = {
                ...
                'formatters': {
                    'intermediate': {
                        '()': 'appsaabaza.apps.common.log.ColorFormatter',
