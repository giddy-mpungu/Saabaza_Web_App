...
from appsaabaza.apps.django_gpg.tests.literals import TEST_KEY_PRIVATE_PASSPHRASE
from appsaabaza.apps.django_gpg.tests.mixins import KeyTestMixin
from appsaabaza.apps.document_states.literals import WORKFLOW_ACTION_ON_ENTRY
from appsaabaza.apps.document_states.tests.mixins import WorkflowTestMixin
from appsaabaza.apps.documents.tests.base import GenericDocumentViewTestCase
...
class DocumentSignatureWorkflowActionTestCase(
    ...
    def test_document_signature_detached_action_via_workflow(self):
        ...
        self.test_workflow_state_2.actions.create(
            ...
            enabled=True,
            action_path='appsaabaza.apps.document_signatures.workflow_actions.DocumentSignatureDetachedAction',
    ...
    def test_document_signature_embedded_action_via_workflow(self):
        ...
        self.test_workflow_state_2.actions.create(
            label='test action', when=WORKFLOW_ACTION_ON_ENTRY,
            enabled=True,
            action_path='appsaabaza.apps.document_signatures.workflow_actions.DocumentSignatureEmbeddedAction',
            action_data=json.dumps(
                obj={
                    'key': self.test_key_private.pk,
                    'passphrase': TEST_KEY_PRIVATE_PASSPHRASE
