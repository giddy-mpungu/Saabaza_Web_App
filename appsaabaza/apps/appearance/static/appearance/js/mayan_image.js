...
class AppsaabazaImage {
    ...
    static intialize (options) {
        ...
        $('img.lazy-load').lazyload({
            appear: function(elements_left, settings) {
                new AppsaabazaImage({element: $(this)});
            },
            threshold: 400,
        });
        ...
        $('.lazy-load').one('load', function() {
            ...
            clearTimeout(AppsaabazaImage.timer);
            AppsaabazaImage.timer = setTimeout(AppsaabazaImage.timerFunction, 250);
        });
    ...
    load () {
        ...
        if (dataURL === '') {
            container.html(AppsaabazaImage.options.templateInvalidDocument);
        } else {
            ...
            setTimeout(function () {
                self.element.on('error', function () {
                    ...
                    if (this.complete === false) {
                        ...
                        container.html(
                            AppsaabazaImage.options.templateInvalidDocument
                        );
                    }
                });
            }, 1);
...
AppsaabazaImage.timer = setTimeout(null);
